#ifndef cutsAndUtility_C
#define cutsAndUtility_C
...
Double_t deltaR(double eta1, double phi1, double eta2, double phi2){
  double dR_ = sqrt( (eta1-eta2)*(eta1-eta2) + (phi1-phi2)*(phi1-phi2) );
  return dR_;
};

Double_t deltaPt(double pt1, double pt2){
  double dPt_ = abs(pt2-pt1)/pt1;
  return dPt_;
};

Bool_t isTriggerMatched(TLorentzVector* p4, map<TString,map<TString,vector<double>*>> &valVect, int itree_, double dR, double dPt)
{
  ...
  int etavar =1;
  int phivar = 2;
  //if(abs(p4->Eta())>=1.8) dR += 0.1;
  for(int isize=0; isize<valVect[itree_][etavar]->size(); isize++){
    double dRtmp = deltaR(p4->Eta(), p4->Phi(), valVect[itree_][etavar]->at(isize), valVect[itree_][phivar]->at(isize));
    double dPttmp = deltaPt(p4->Pt(), valVect[itree_][ptvar]->at(isize));
    if(dRtmp<dR && dPttmp<dPt){
      dR = dRtmp;
      dPt = dPttmp;
      closestFound = true;
      iMatched = isize;
  ...
  if(closestFound == false || iMatched ==-1) return false;
  else if(closestFound == true && iMatched !=-1) return true;
}
